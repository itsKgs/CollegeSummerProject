...
# ╔═╡ 5944230a-4199-4082-83e9-f4b34adedb01
import Pkg

# ╔═╡ 163ebdc0-50a8-4c06-b965-fc8c2076d411
using ModelingToolkit, MethodOfLines, OrdinaryDiffEq, DomainSets, Plots

# ╔═╡ c3b1c7d0-41f0-11f0-39f7-61967d600586
md"""
# MethodOfLines.jl
"""

# ╔═╡ a893a46a-e1c9-43b9-9126-4f96d49da4f6
md"""
## Using the **Brusselator PDE** as an example
"""

# ╔═╡ 24123628-cd33-4cd4-80e5-c4bad8566b20
@parameters x y t

# ╔═╡ 3b3e4329-bb5a-46f3-bc71-1f9d305b5c5f
@variables u(..) v(..)

# ╔═╡ e3bfb205-4131-4587-bf98-37072c1e1b73
Dt = Differential(t)

# ╔═╡ 55174420-7533-4770-8391-f2f89e9de452
Dx = Differential(x)

# ╔═╡ 6b05445b-7d82-4d59-b33b-3a41644ff96c
Dy = Differential(y)

# ╔═╡ 42cae84e-263b-41f7-99b9-2f16c2a2e2fd
Dxx = Differential(x)^2

# ╔═╡ 7f88bb56-aeb9-458b-a7c2-c89400e82b6f
Dyy = Differential(y)^2

# ╔═╡ de0cdf7b-dc68-4ba6-962d-d75e35441110
# Laplacian operator (scalar Laplace)
∇²(u) = Dxx(u) + Dyy(u)

# ╔═╡ 9cfffdd6-5d63-421f-94b0-9afb2fd4dc28
# Forcing function
brusselator_f(x, y, t) = (((x - 0.3)^2 + (y - 0.6)^2) <= 0.1^2) * (t >= 1.1) * 5.

# ╔═╡ dce80f41-4279-41eb-88e1-699b1410f165
# Domain limits
	x_min = y_min = t_min = 0.0

# ╔═╡ 7ef081a7-0f6e-4ff1-a4e9-a5acc85b49a4
x_max = y_max = 1.0

# ╔═╡ f1fec333-9635-4649-833f-0acfa3148a63
t_max = 11.5

# ╔═╡ 3af4bcb7-1aa6-4347-ae44-47fcead114b9
# Diffusion coefficient
α = 10.0

# ╔═╡ 59abb4d3-23f9-47f7-b768-0773279fd8ba
# Initial conditions
u0(x, y, t) = 22 * (y * (1 - y))^(3/2)

# ╔═╡ 84e10567-5188-46ae-912b-2dbd0ed21b55
v0(x, y, t) = 27 * (x * (1 - x))^(3/2)

# ╔═╡ d8dca1f7-06ad-4366-b0f0-b679cd73cc39
# 2. PDE system
eqs = [
...
# ╔═╡ 9416a4f1-a5f6-4ab3-b4f2-1a11df620220
# 3. Domains
domains = [
...
# ╔═╡ b03dff84-23e5-4016-8682-05ea93caa731
...
# ╔═╡ 213153ed-0b8b-4e69-9cb3-81bb7c862dd8
@named pdesys = PDESystem(eqs, bcs, domains, [x, y, t], [u(x, y, t), v(x, y, t)])

# ╔═╡ d7f2b58b-ba6f-41da-8ffb-791af6a4923d
# 5. Discretize using MethodOfLines (MOL)
N = 32

# ╔═╡ 9f9f54c2-4124-4d38-a166-96abd17b49e0
discretization = MOLFiniteDifference([x => N, y => N], t, approx_order=2)

# ╔═╡ 4749324b-c4fb-4445-8fed-0bf0880bc8e8
# 6. Convert PDE to ODE system
@time prob = discretize(pdesys, discretization)

# ╔═╡ de819d83-1d9f-4970-bbf5-d3b652e7b8bd
sol = solve(prob, TRBDF2(), saveat=0.5)

# ╔═╡ 7c1bc572-e9fc-4dde-a6e3-7ad31f56a774
discrete_x = sol[x]

# ╔═╡ 356a294c-0247-4a58-b024-de8bfd435a0c
discrete_y = sol[y]

# ╔═╡ a359095f-ccf9-4f83-b8ed-b4778e2a5f02
discrete_t = sol[t]

# ╔═╡ 53744b30-71a7-4d82-8d5b-3f4c0ff18878
solu = sol[u(x, y, t)]

# ╔═╡ 65a831f0-e6b2-43af-b7dc-20fb5b4586fe
solv = sol[v(x, y, t)]

# ╔═╡ c1486c29-170e-4121-9f72-37487aa63e39
...
# ╔═╡ 0a4b34e6-8c0a-4f2f-9b8b-0eda6cca5cca
...
[compat]
DomainSets = "~0.7.15"
MethodOfLines = "~0.11.8"
ModelingToolkit = "~9.80.3"
...
manifest_format = "2.0"
project_hash = "649ae7d9dd7028c26d54e64c7287a1f8ef6a3d90"
...
[[deps.MethodOfLines]]
deps = ["Combinatorics", "DiffEqBase", "DomainSets", "IfElse", "Interpolations", "Latexify", "LinearAlgebra", "ModelingToolkit", "OrdinaryDiffEq", "PDEBase", "PrecompileTools", "RuntimeGeneratedFunctions", "SciMLBase", "StaticArrays", "SymbolicIndexingInterface", "SymbolicUtils", "Symbolics", "TermInterface"]
git-tree-sha1 = "88a79939cb2df439648bb18cc6fa40086c6c31f7"
...
git-tree-sha1 = "d16d965cf797e0843780ea2b4a0c695de8f506c6"
...
git-tree-sha1 = "467fa575f9c268d00f663ca63589757dd983195f"
